...
export interface ItemQualityView {
  ...
  /**
   * Поводы усомниться в числах. `mixedAnonymity` (FR-43) — среди учтённых партий импорта есть
   * и сырые, и предобезличенные выгрузки; поля может не быть у ответов ручки до этого
   * требования.
   */
      ...
      */}
      {view.bias?.mixedAnonymity ? (
        <Banner
          ...
          description="Среди учтённых выгрузок есть и сырые, и предобезличенные. Ключ участника в них считается по-разному, поэтому одно и то же лицо попадает в выборку под двумя ключами: число респондентов завышено, а пороги достоверности достигаются раньше, чем на самом деле. Снимите с учёта партии одного из видов на вкладке импорта."
        />
      ) : null}

...
- [ ] **Step 1:** падающие тесты — последовательность замеров, разбор ответов заглушки LMS,
      вывод словами; обрезанное значение отличается от отвергнутого.
- [ ] **Step 2:** `scripts/scorm/probe/probe-runner.js` — чистая логика замеров над
      переданным API, без DOM.
- [ ] **Step 3:** `npm test -- tests/stand-probe.test.ts`

## Task 2: Пакет-зонд

- [ ] **Step 1:** падающие тесты — манифест валиден, страница подключает замерный модуль,
      ZIP несёт ровно два файла.
- [ ] **Step 2:** `scripts/scorm/generate-probe-scorm.ts` + `npm run scorm:probe`.
- [ ] **Step 3:** `npm test -- tests/stand-probe-package.test.ts`

## Task 3: Прогон на локальном плеере

- [ ] **Step 1:** пакет открывается в `npm run scorm:player`, все шесть блоков отчёта
      заполняются, кнопка копирования отдаёт JSON.
- [ ] **Step 2:** починка того, что не работает в локальном рантайме.

## Task 4: Инструкция и передача

- [ ] **Step 1:** инструкция по прогону на стенде: загрузка, запуск, снятие отчёта, что

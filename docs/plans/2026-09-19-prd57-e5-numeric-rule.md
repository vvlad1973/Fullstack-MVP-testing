...
- [ ] **Step 3: Проверка** — `npm test -- tests/answer-check-number.test.ts`
...
- [ ] **Step 3: Проверка** — `npm test -- tests/answer-rules-block.test.tsx`
...
- [ ] **Step 3: Проверка** — оба набора плюс `tests/scoring-engine-port.test.ts` (паритет с
...
- [ ] **Step 3: Проверка** — `npm test -- server/services/analytics/__tests__/answer-spread.test.ts`

## Task 8: Приёмка

- [ ] **Step 1:** `npx tsc --noEmit --incremental false` и затронутые наборы целиком.
- [ ] **Step 2:** живой прогон в браузере: числовое правило с допуском, диапазон двумя правилами,
      ответ дробью, «ожидается число», ступенчатый балл, разбор попытки, гистограмма на вкладке
      «Вопросы».
- [ ] **Step 3:** пакет: `npm run scorm:template`, локальный плеер, то же задание.

...
```tsx
it("предел длины вводится и не превышает системного потолка", () => {
  const seen: number[] = [];
  ...
  expect(screen.getByText(/не больше 250/i)).toBeTruthy();
  cleanup();
});
```
...
«До скольких символов участник может ответить. Пусто — {потолок}». Потолок читается из
конфигурации клиента тем же способом, каким это делают соседние экраны.

Число больше потолка не принимается: показывается ошибка поля, сохранение блокируется
существующим баннером `banner-question-validation` — новый механизм валидации не заводится.
...
Хост передаёт предел из `dataJson.maxLength`, а при его отсутствии — из системного потолка;
рендерер сам в конфигурацию не ходит, он чистый.

...
| Верх формы | Баннер: сколько проблем и в чём они, плюс действие «Перейти к ошибкам» / «Перейти к предупреждениям» |
| Сам элемент | Ошибка — поле помечено невалидным; предупреждение — знак тревоги рядом со ЗНАЧЕНИЕМ, с подсказкой |
...
Реализовано для ошибок: точка на вкладке, сводный баннер и переход по `goToError`
(`client/src/features/tests/editor/test-editor.tsx`). Приводится в соответствие: точка на пункте
рейла и участие ПРЕДУПРЕЖДЕНИЙ в обоих индикаторах и в баннере.

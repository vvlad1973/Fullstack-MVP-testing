...
Цена, принятая сознательно: `POST` прошёл, а дозапись упала — тест создан без части
настроек. Профиль риска у всех строк таблицы одинаковый, но сообщают о сбое по-разному:
дозапись показателей, шкал, оценки и оформления идёт ВНУТРИ мутации создания, поэтому
её сбой роняет мутацию — ящик остаётся открытым с баннером ошибки, хотя тест уже создан
(повторное «Сохранить» создаст второй — это давняя ловушка этого пути, не новая).
Структура дописывается ПОСЛЕ мутации, когда ящик уже закрывается по появлению
идентификатора, поэтому её сбой сообщается всплывающим сообщением: «Тест создан, но
структуру дописать не удалось».

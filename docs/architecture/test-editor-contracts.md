...
| `runtime.showSectionResults` / `show_section_results` | `boolean` | `true` | FR-05a | Показывать экран промежуточных итогов раздела (опциональный системный узел, только секционные тесты). Неприменимо к `linear_flat` (нет разделов) — рантайм там игнорирует поле. Правится в двух местах: переключателем «Показывать итоги раздела» («Обратная связь и итоги» → «Во время теста») и пунктом «Скрыть»/«Показать» в меню строки полотна — это ОДНА настройка |
| `hidden` (страницы) / `content_pages.hidden` | `boolean` | `false` | — | Экран есть в тесте, но ученику не выдаётся (решение владельца 2026-09-20). Скрыть можно любую карточку полотна, кроме `questions` и `router`. Поток авторских страниц и `intro` фильтрует `shared/flow/page-sequence`; `start` / `results` / `review` спрашивают `isSystemScreenHidden` оттуда же |
...
- **Баннер НАЗЫВАЕТ проблему, а не только считает её.** Печатается сообщение первой находки, а
  хвостом — «Ещё N полей»; остальные автор увидит на месте, когда исправит эту. Один счётчик
  оставлял автора ни с чем, если виноватое поле лежит в свёрнутой карточке: на экране
  «Поля с ошибками: 1» и ни одной пометки.
- **Сворачиваемая карточка сама говорит об ошибке внутри себя** — точкой в шапке, тем же знаком
  `tb-status-dot`, что у карточки шкалы. Её тело живёт только в развёрнутом виде, поэтому
  свёрнутая карточка без точки молчит о проблеме, которую баннер уже посчитал.
...
### Пометка на КАЖДЫЙ адрес (2026-09-22)

Баннер называет ОДНУ ошибку — первую по порядку проверки — и считает остальные («Ещё N
полей»). Решение владельца 2026-09-22: порядок оставлен как есть. Отсюда следствие, которое
до этого не выполнялось: раз баннер молчит про остальные, каждая из них обязана быть помечена
на своём месте, иначе ошибка блокирует сохранение и нигде не написана. Так и вышло у нового
теста: баннер говорил «Название обязательно», а вторая ошибка — «Добавьте хотя бы одну
тему» — жила одной точкой в рейле.

Пометка выбирается по тому, есть ли у адреса поле на экране:

| Что за адрес | Чем помечается |
| --- | --- |
| Поле формы | `error` у самого контрола (`Input` / `Select` / `NumberInput` / `Textarea` / `RadioGroup`) |
| Адрес есть, а поля на экране нет (контейнер, свёрнутая карточка, скрытый контрол) | Красный `Banner` на месте — и точка `tb-status-dot--err` в шапке карточки, чтобы свёрнутая говорила о себе |

Правило «пометка сидит на поле» не выполняется молча в трёх случаях, и каждый требует
баннера-заместителя: **контейнер** (нет тем, нет включённой лестницы), **карточка целиком**
(находки профиля у показателя) и **скрытый контрол** — «управление статусом» показывается
только булеву показателю, а ошибка «тип не булев, а управление задано» возникает именно у
небулева, то есть поля, которое надо пометить, на экране нет.

Три ловушки этого места:

- **Местная проверка секции не отменяет пометку.** Шкалы и уровни шкал считают свои ошибки
  сами (`keyErrorOf`, `draftErrors`), адаптивный уровень — тоже (`isValid`). Такая точка
  обязана слушать И общий индекс: у свёрнутой адаптивной темы точка бралась только из
  местного счёта и оставалась ЗЕЛЁНОЙ при сломанном уровне внутри.
- **Прямой ребёнок `.tb-settings-content` становится ЛИПКИМ баннером шапки панели** (правило
  в `tb-components.css`, отрицательная верхняя отбивка). Баннер ошибки, поставленный туда,
  наезжает на соседние кнопки — его место внутри карточки.
- **Индекс ошибок надо ДОВЕЗТИ.** `ScenarioSettingsPane` и `AdaptivePane` пропа `fieldErrors`
  не получали вовсе, поэтому подсвечивать им было нечем.

Гард — `client/src/features/tests/editor/sections/__tests__/error-highlighting.test.tsx`:
каждый случай идёт через настоящую проверку (`модель -> validateTestEditor ->
buildFieldErrorIndex -> секция`), поэтому ломается и когда пропала пометка, и когда у
проверки сменился адрес.

### Переход к проблеме: как он находит адрес (2026-09-22)

Якорь — атрибут `data-field` с адресом находки. Правила отбора и раскрытия:

- **Самый ДЛИННЫЙ путь-предок, а не первый в DOM.** Адреса вложены: у темы есть общий `sections`
  (кнопка «Добавить тему»), `sections[i]` (карточка) и `sections[i].drawCount` (поле). Отбор по
  первому совпадению уводил переход к кнопке добавления темы — она стоит в разметке ВЫШЕ карточек,
  и автор с ошибкой внутри темы приезжал к созданию новой (`findFieldAnchor` в `test-editor.tsx`).
- **Свой якорь у каждого блока со своей проверкой.** У темы это `sections[i]`,
  `sections[i].drawCount`, `sections[i].drawBlueprintJson`, `sections[i].formSetJson`: без них
  переход к квотам и вариантам упирался в адрес темы целиком.
- **Раскрытие — по событию, а не по догадке.** Переход шлёт на якоре всплывающее `tb:reveal`
  (`REVEAL_EVENT` в `field-errors.ts`), сворачиваемая карточка его слушает и открывается;
  вложенность разбирается всплытием. Угадывать раскрытие по DOM нельзя: тело свёрнутой карточки
  остаётся в разметке (её прячет CSS), поэтому «точный» якорь находится, а `focus()` на скрытом
  элементе молча не срабатывает — до этого нажатие «Перейти к ошибкам» не двигало ничего.
- **Фокус — на поле, а не на первой кнопке блока.** У числового поля DS стрелки
  «Уменьшить»/«Увеличить» стоят в разметке перед `input`, и фокус доставался действию, меняющему
  значение.

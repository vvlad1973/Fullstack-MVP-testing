...
9. `designSettingsJson` пишется ТОЛЬКО при создании и только как `{ templateId }` — из
   `model.design`, среза, который есть лишь у черновика НОВОГО теста (`emptyEditorModel`;
   `apiToEditorModel` его не заполняет). У существующего теста оформление правит свой ресурс
...
## Черновик нового теста не требует сохранения

Правило: любая настройка теста доступна ДО первого сохранения. Автор набирает её в
черновике редактора, и она записывается одним действием «Сохранить» вместе с самим тестом.
Экран, который отвечает «сначала сохраните черновик», — дефект, а не ограничение.

Механика: тело `POST /api/tests` несёт только то, что лежит в колонках строки теста. Всё
остальное — отдельные ресурсы, и они дописываются СРАЗУ ПОСЛЕ INSERT теми же маршрутами,
какими правятся у существующего теста (мутация создания в `useTestEditor`):

| Что | Чем дописывается | Почему не телом создания |
| --- | --- | --- |
| Показатели | `result-variables-api` | своя таблица, свой CRUD |
| Шкалы, вклады вопросов | `scales-api` | вклады ссылаются на шкалы по ключу — нужен порядок |
| Оценка отдельных вопросов | `scoring-api` `saveQuestionOverrides` | `test_question_scoring`, ключ — (тест, вопрос) |
| Оформление, кроме выбора шаблона | `PUT /api/tests/:id/design` | там уже написана проверка против манифеста |
| Структура страниц | `useContentPages.commit(createdId)` | системные узлы создаёт сама транзакция; авторские страницы дописываются к ним |

### Структура страниц нового теста

Особый случай: системные узлы (`start`, `questions`, `review`, `results`,
`section-results`, `intro`, `router`) создаёт транзакция INSERT — до неё их в базе нет.
Чтобы вкладка работала раньше, редактор их ПРЕДСКАЗЫВАЕТ: зовёт тот же планировщик,
`shared/content-pages/lifecycle` `planSystemPages([], …)`, с тем же входом (сценарий,
темы, каталог шаблона + каталог встроенного). Планировщик живёт в `shared/` именно
поэтому: вторая реализация разошлась бы с серверной, и автор увидел бы перед
сохранением одну структуру, а получил другую. Раскладку `position`/`type`/`mode` по
виду узла оба хоста тоже берут оттуда (`positionForKind`, `legacyTypeForKind`).

Предсказанные строки несут ЧЕРНОВЫЕ идентификаторы. При сохранении `commit(createdId)`
читает настоящие страницы созданного теста и сопоставляет их с предсказанными по паре
«вид + тема» (`adoptRealSystemIds`) — той же, по которой их различает планировщик.
Дальше работает ТОТ ЖЕ разбор, что у существующего теста: авторские страницы
создаются, изменённые системные узлы обновляются, порядок выравнивается. Серверная
строка, которой не нашлось предсказания, добавляется в черновик, а не удаляется:
сервер здесь главный.

Состав узлов зависит от сценария и набора тем, и автор меняет их по ходу. Предсказание
пересогласовывается тем же планом «создать/удалить», а не собирается заново, — иначе
смена состава тем стирала бы уже выбранные варианты и заполненные поля узлов. Сервер
на своей стороне ведёт себя точно так же.

Исключение — `designSettingsJson.templateId`: он ОБЯЗАН уехать телом создания, потому что
системные `content_pages` раскладывает и связывает с шаблоном та же транзакция, что вставляет
строку теста. Прочие поля оформления в маршрут создания не принимаются намеренно: второй путь
их записи означал бы вторую, неизбежно расходящуюся валидацию.

### Сбой дозаписи: повтор не создаёт второй тест

`POST` прошёл, а дозапись упала — тест в базе уже есть. Повтор сохранения ОБЯЗАН
дополнить его, а не создать дубль, поэтому идентификатор запоминается ссылкой сразу
после успешного `POST`, до любой дозаписи (`useTestEditor`, `createdIdRef`):

- пока ссылка не пуста, создание не повторяется — тело уходит `PUT`'ом по этому
  идентификатору, с версией, прочитанной перед самой записью (правки, сделанные после
  неудачной попытки, так не теряются), а дозапись прогоняется заново; её маршруты
  перезаписывают своё состояние целиком, поэтому повторный прогон приводит к тому же
  результату, что удачный первый;
- ссылка живёт РОВНО один сеанс создания и обнуляется при смене `sessionKey` (закрыли
  ящик — следующее создание начинается чисто, а не дописывается в прошлый тест);
- `consumeCreatedId` ссылку НЕ трогает: её читает дозапись структуры уже после того,
  как мутация выставила `createdId`, и обнуление там устроило бы гонку, в которой
  структура нового теста уходит в никуда.

Сообщают о сбое по-разному, потому что дозапись идёт в двух местах. Показатели, шкалы,
оценка и оформление пишутся ВНУТРИ мутации: её сбой оставляет ящик открытым, и баннер
ошибки в режиме создания прямо говорит, что тест создан, а повтор дополнит его же.
Структура дописывается ПОСЛЕ мутации, когда ящик уже закрывается по появлению
идентификатора, — её сбой сообщается всплывающим сообщением «Тест создан, но структуру
дописать не удалось».

Сохранённого теста по-прежнему требует ОДНА вкладка — «Комментарии» (PRD-52): рецензировать
то, чего нет, нечем, и вкладка в режиме создания скрыта.
...
Три действия, они стоят ВСЕГДА и запираются по смыслу: прыгающий состав кнопок хуже
запертой кнопки — кнопка остаётся на своём месте, а состав нет.

| Элемент | Поведение |
| --- | --- |
| «Отменить» | откатывает черновик и закрывает ящик СРАЗУ, без вопроса: автор уже сказал, чего хочет. Заперта, когда откатывать нечего |
| «Применить» | записывает набранное и оставляет ящик ОТКРЫТЫМ. Активна только при dirty и без ошибок. В режиме создания создаёт тест, и ящик переключается на созданный (`onCreated`) |
| «Закрыть» | выход. Есть неприменённое — тот же диалог, что у крестика в шапке (§8.4) |
| «Изменено» + «Показать изменения» | видны только при dirty; вторая открывает сводку по вкладкам |
| «Сбросить всё» | НЕ присутствует (FR-05b) |

Решение владельца (2026-09-23): сохранение перестало закрывать ящик, выход стал
отдельным действием. Прежний подвал менял состав («Закрыть» + «Сохранить» без правок,
«Отменить» + «Сохранить» с правками) и не давал применить, не выходя.

...
**Версия:** 3.2.0 (см. [«История версий»](#история-версий))  
...
| --- | --- | --- |
| 3.2.0 | 2026-09-11 | **Параметр умеет показать варианты картинками и попасть в атрибут сцены** (§6). `optionPreviews` — путь к файлу пакета на каждый вариант `select`: редактор рисует карточки с изображениями вместо выпадающего списка, потому что выбирают ОБЛИК, а словом его не описать. `dataAttr` — имя атрибута вида `data-имя`, в который оба хоста кладут выбранное значение на корне сцены: кастомное свойство — только значение, по нему нельзя ВЫБРАТЬ правило, а шаблон, меняющий картинку по варианту, выбирает селектором. Версия МИНОРНАЯ: оба поля необязательны, шаблон без них работает как прежде — `select` остаётся списком, атрибут не ставится. Кривое имя атрибута и отсутствующий файл превью блокируют импорт, а не всплывают молчаливой пустой карточкой при прохождении. |
...
### Выбор картинкой: `optionPreviews`

Параметр `select` вправе объявить картинку на вариант — путь к СВОЕМУ файлу пакета:
...
Тогда редактор рисует не выпадающий список, а карточки с изображениями. Смысл в том,
что выбирают ОБЛИК: словом «B2O» его не описать, и без картинки автор выбирал бы
вслепую и шёл сверяться в предпросмотр.

Правила:

- пути — обычные ссылки манифеста: файл обязан быть в пакете (иначе импорт
  блокируется как `FILE_MISSING`), внешний `http(s)`-адрес запрещён, файл не
  считается неиспользованным;
- превью у варианта, которого нет в `options`, — предупреждение: картинка
  игнорируется, выбор работает;
- `optionPreviews` у типа, отличного от `select`, — предупреждение, поле не
  учитывается;
- вариант без картинки рисуется карточкой с пустой миниатюрой и подписью;
- поле необязательное: без него `select` остаётся списком, как и раньше.

### Значение как атрибут сцены: `dataAttr`

`cssVar` кладёт значение параметра в кастомное свойство. Свойство — только ЗНАЧЕНИЕ:
по нему нельзя выбрать правило, а шаблон, меняющий картинку по выбранному варианту,
выбирает именно селектором. Для этого параметр объявляет `dataAttr`:
...
Оба хоста кладут значение в этот атрибут на КОРЕНЬ сцены — там же, где живут
CSS-переменные параметров и тема (`data-theme`, §6.1). В пакете это `<html>`, в вебе —
host теневого дерева. CSS шаблона выбирает по нему:
...
- имя обязано быть вида `data-имя` (строчные латинские буквы, цифры, дефис) — иначе
  импорт блокируется (`PARAM_DATA_ATTR_INVALID`): хост произвольное имя не пишет, и
  шаблон молча рисовал бы запасной вариант;
- значение приводится к строке как есть; у числа единицы НЕ дописываются — это не
  размер, а ключ;
- нет значения у теста — берётся `default` манифеста; нет и его — атрибут не ставится;
- поле необязательное и обратно-совместимое.

Селектор, рассчитанный на оба хоста, пишется парой — в вебе класс/атрибут темы висит
на host снаружи теневого дерева, и потомковый селектор до него не достаёт:

...
страницу отчёта о результатах (`kind: "report"` / `report.adaptive`, §8.4) с постраничной
раскладкой и принудительным разрывом листа (§8.4.6).
...
карточке темы — §10.6. Ещё не действующие возможности вынесены в §3.  
**Версия:** 1.6.0 (см. [«История версий»](#история-версий))  
...
действительно печатают: список собирается сканированием этих макетов на прямые пути
...
Раздел манифеста верхнего уровня объявляет по каждому экрану, какие из четырёх подблоков
итогов (`summary`, `scales`, `indicators`, `topics`) печатаются и в каком порядке.
...
Раздел необязателен. Шаблон, не объявивший его, получает зашитое умолчание — тот порядок,
что макет печатал до появления этой настройки (`summary`, `scales`, `indicators`, `topics`) —
поэтому вид уже собранных тестов не меняется сверх переезда самих заголовков на зонтик и
подзаголовки.
...
Ключ вне закрытого перечня (`summary` / `scales` / `indicators` / `topics`) отбрасывается на
любом слое без ошибки: раздел — данные, а не код, и обязан принимать значение, не роняя рендер.
...
на стартовом экране в кулдауне). Это не снимок экрана результатов, а самостоятельная
страница со своей вёрсткой, и вёрстка эта принадлежит ШАБЛОНУ, а не ядру.

Страницу рисует тот же браузерный рендерер, что и ученические экраны; конвейер экспорта
растеризует её и укладывает в A4. Ядро отвечает за данные и за файл, шаблон — за страницу.
Данные, из которых страница собирается, — публичный контекст отчёта (§10.2).
...
- `placeholders[]` — НЕ применяются: в отчёте нет содержимого, которое обучающийся читает
  как страницу. Объявленный непустой `placeholders[]` — ошибка валидации (§17.1).
...
`isTopics`) — DSL не умеет сравнивать строки (§9), поэтому выбор ветки шаблону нужен уже
готовым. Макет обходит массив ОДНИМ циклом вместо четырёх фиксированных секций:
...
решала его сама. Полей вердикта ключа (`passed`, `passClass`, `statusLabel`) в строке пока нет:
их нечем наполнить, пока у ключа нет порога.
...
- невалидные объявления вариантов отчёта (§8.4): вариант без `key` или без `layoutFile`,
  ...
  `placeholders[]` у варианта отчёта, не ровно один `isDefault` на объявленный вид;

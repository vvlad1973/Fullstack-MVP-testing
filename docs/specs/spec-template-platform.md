...
**Версия:** 3.0.0 (см. [«История версий»](#история-версий))  
...
**Вердикта в строке НЕТ** (решение владельца 2026-09-03): подтема считается и показывается,
но не судится. С версии 3.0.0 из строки убраны `passed`, `passClass` и `statusLabel` — они
были в контракте 1.7.0-2.0.0. Подтема говорит о результате — сколько вопросов, сколько
баллов, какая доля, — а вердикт выносит ТЕМА, и карточка вокруг строки его уже объявила.
Макет, который продолжает связывать снятые поля, ничего не печатает для них: класс выходит
пустым, гейт `{{#if statusLabel}}` — всегда ложным.

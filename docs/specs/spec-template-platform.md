...
ширины САМОЙ полосы: полоса шириной `p` показывает ровно левую часть рампы, растянутой на всю
дорожку, поэтому короткая полоса не заканчивается благоприятным цветом. Изгиб рампы (середина,
дуга оттенка) передаётся промежуточными точками. Цвета в заливке — только разобранные ядром
тройки HSL: испорченное значение своей схемы заменяется краем светофора, а не печатается.

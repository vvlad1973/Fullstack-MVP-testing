...
карточке темы и окраска его полос — §10.6, блоки разделов — §10.7, сводный разрез по тесту — §10.8, надпись лимита
времени — §10.9, толкование темы и подтемы — §10.10, заголовки итога — §10.11. Ещё не
действующие возможности вынесены в §3.  
**Версия:** 3.13.0 (см. [«История версий»](#история-версий))  
...
| --- | --- | --- |
| 3.13.0 | 2026-09-24 | **Признак «выход закрывает раздел»** (§10.9, PRD-67). В контекст стартового экрана добавлено поле `course.closesOnLeave`: `true`, когда автор включил настройку «Закрывать раздел при выходе» и у теста есть хоть один лимит времени. Выход из начатого раздела тогда закрывает его навсегда, а в тесте без разделов завершает попытку. Макет может предупредить ученика ДО начала; поставляемые шаблоны признак не печатают — оба хоста предупреждают уведомлениями ядра. Версия МИНОРНАЯ и поле НЕОБЯЗАТЕЛЬНОЕ: у теста без настройки его в контексте нет, контекст байт в байт прежний. |
| 3.12.0 | 2026-09-24 | **Окраска полос подтем — настройка автора, заливку считает ядро** (§6, §10.6, §10.8). Параметр оформления `breakdownBarFill` выбирает, что кодирует цвет полосы: исход (`verdict`, прежнее поведение), долю (`share`) или ничего (`neutral`). В режиме «По доле» строка разреза получает готовую заливку `barFill` — прямой градиент между цветами схемы уровней теста до цвета в точке доли — и пустой `passClass`; макет печатает её в `style` полосы. Параметр один на экран итогов и отчёт, поэтому они больше не расходятся. Там же `default` манифеста начал действовать для параметров, которые ядро читает само (схема уровней, вид шкал и показателей, окраска полос): шаблон задаёт вид нетронутого теста, а редактор показывает применяемое значение. Версия МИНОРНАЯ: без параметра строка байт в байт прежняя, а у поставляемых шаблонов умолчания манифеста и ядра совпадали. Все три поставляемых шаблона печатают `barFill`; «Сертификация» по умолчанию окрашивает полосы по доле в брендовой паре цветов, а её приём с `background-size` в `report.css` снят. |
| 3.11.0 | 2026-09-24 | **Ссылка `<a href>` в отчёте нажимается и в PDF** (§8.4.5). Конвейер экспорта накладывает настоящие ссылки PDF поверх растра не только по чипам `.pdf-link-btn[data-url]`, но и по каждой `<a href>` с адресом `http(s)://` или `mailto:` — прежде всего по ссылкам, которые автор вставил в текст страницы документа, вводного блока или рекомендаций. Область строится по строкам ссылки (`getClientRects`), поэтому перенесённая ссылка нажимается на каждой строке и не захватывает соседний текст; якорь `#…` и относительный путь не переносятся. До этого такая ссылка в PDF выглядела ссылкой и не нажималась. Версия МИНОРНАЯ: от шаблона ничего не требуется — авторский `richText`, напечатанный как есть, получает рабочие ссылки сам, а контракт `.pdf-link-btn` действует по-прежнему. |
| 3.10.0 | 2026-09-21 | **Толкование темы и подтемы, заголовки итога** (§10.10, §10.11). В контекст строки темы добавлены `interpretationHtml` — готовая разметка текста, который ОБЪЯСНЯЕТ результат, — и такое же поле у строки разреза; рядом два признака раскладки, `hasBreakdownNotes` и `hasAside`, которые ядро считает за макет (движок не умеет ни перебрать строки, ни сложить два вопроса в один ответ, §9). Толкование отличается от обратной связи ПРАВИЛОМ ВЫДАЧИ: оно печатается при любом вердикте, стоит при своей теме или полосе и в сводный блок «Рекомендации» не попадает. Отдельно вариант `results.*` получил три настройки заголовков итога (`headingDocument` / `headingPassed` / `headingFailed`): тест называет свой итог сам, а НОВЫХ полей контекста при этом не появляется — значения приходят в прежние `course.title`, `result.statusLabel` и `report.verdictHeadline`. Версия МИНОРНАЯ, все поля НЕОБЯЗАТЕЛЬНЫЕ: тест, ничего не заполнивший, даёт контекст байт в байт прежний, а шаблон, новых полей не связывающий, работает как работал. Все три поставляемых шаблона переведены. |
| 3.9.0 | 2026-09-21 | **Подзаголовок раздела настраивает АВТОР ТЕСТА, а не вёрстка** (§10.1, PRD-22 §11). Строка над авторской инструкцией на экране «Введение раздела» была жёстким словом «Инструкция» в макете. Вариант вида `intro` объявляет её двумя обычными настройками страницы — тумблером `sectionSubtitleShown` (`boolean`, умолчание `true`) и формулировкой `sectionSubtitle` (`text`, умолчание «Инструкция»), — а ядро отдаёт разрешённую строку макету как `page.sectionSubtitle` в том же неймспейсе, где живёт `page.nextLabel`. Гасит надпись ТОЛЬКО тумблер: пустое поле означает «следовать шаблону» и печатает умолчание, поэтому макет проверяет строку на непустоту (`{{#if page.sectionSubtitle}}`), а не печатает её безусловно. Расширение обратно-совместимое: вариант, не объявивший настроек, печатает то, что стоит в его макете, а страница без сохранённых значений — умолчание шаблона. |
| 3.8.0 | 2026-09-20 | **Контекст итогов несёт признак «часть ответов на проверке»** (§7.3, PRD-57 FR-41). У `result` появилось поле `pendingReview`: `true`, когда в попытке есть ответ, который никто не проверял (развёрнутый ответ §5; ответ, чьё выражение не уложилось в бюджет §6.4). Текст и место — за шаблоном: контракт несёт ФАКТ, а не формулировку. Поле заводится ДО появления самой проверки намеренно — добавить его позже значило бы поднять версию ещё раз и обновить внешние шаблоны из реестра PRD-3. Версия МИНОРНАЯ и поле НЕОБЯЗАТЕЛЬНОЕ: у теста без открытых заданий контекст остаётся байт в байт прежним, а шаблон, ничего не знающий о признаке, работает как работал. |
| 3.7.0 | 2026-09-19 | **Слот текста задания перестаёт быть заголовком** (§8, PRD-57 FR-03d). Тег `<h2 class="tb-scene__qtitle" data-slot="question-text">` заменён на `<div class="tb-scene__qtitle" role="heading" aria-level="2" data-slot="question-text">`. Причина в HTML, а не во вкусе: в заголовке допустим только строчный контент, поэтому блочный листинг кода и блочная формула туда невалидны — а именно их приносят PRD-57 §4.1 и §4.2. Вид на экране не меняется, роль для экранного диктора сохраняется явно. Версия МИНОРНАЯ: имя слота, его наполнение и контекст прежние, и шаблон, оставшийся с `<h2>`, продолжает работать — он просто не покажет блочный листинг. Все три поставляемых шаблона переведены. |
| 3.6.0 | 2026-09-18 | **Лимит времени приходит готовой надписью** (§10.9). Рядом с прежним `course.timeLimitMinutes` в контекст добавлено поле `course.timeLimitLabel` — те же минуты, разложенные ядром в дни, часы и минуты («14 дней», «2 ч 30 мин», «45 мин»); той же надписью печатается лимит раздела (`sectionIntro.timeLimitLabel`). Единица измерения переехала из макета в ядро, потому что движок намеренно без выражений и хелперов (§9): макет с зашитым «мин» на двухнедельном бюджете говорил «20160 мин», а его отсчёт — «20160:00». Рантайм обоих хостов печатает отсчёт теми же единицами: `M:SS`, `H:MM:SS` от часа, `D д H:MM:SS` от суток. Версия МИНОРНАЯ: число осталось в контексте, и шаблон, связывающий только его, работает как прежде. Оба поставляемых шаблона переведены на надпись — узел печатает её через `data-path`, а гейт блока стоит на ней же. |
| 3.5.0 | 2026-09-18 | **Внешняя ссылка в авторском тексте сохраняется** (§8.2.1.1). Правило «внешний `http(s)://` в `src`/`href` удаляется целиком» разделено по смыслу: `src` называет ресурс, который страница грузит сама, и остаётся запрещённым везде, а `href` у `<a>`/`<area>` — направление, по которому учащийся идёт сам, и до клика не запрашивает ничего. Такой адрес теперь СОХРАНЯЕТСЯ и получает `target="_blank" rel="noopener noreferrer"`, чтобы переход не увёл кадр с незавершённой попытки; `href` у прочих тегов (в том числе `<base>`) снимается по-прежнему. Прежде панель форматирования предлагала кнопку «Ссылка» (PRD-22 FR-33), а сохранялся `<a>` без адреса — от простого текста не отличить. Версия МИНОРНАЯ: ранее валидный шаблон продолжает работать, но обязан ОФОРМИТЬ ссылку — у `<a>` без адреса браузерного умолчания не было, и правила для неё в шаблоне могло не быть. Оба поставляемых шаблона правило уже несут: области авторской разметки в `theme.css` и общее `.tb-report a` в `report.css`. |
| 3.4.0 | 2026-09-18 | **Описание теста печатается с оформлением** (PRD-59 FR-11). В контекст стартового экрана добавлено парное поле `course.descriptionHtml` рядом с прежним `course.description` (§10.4). Ядро строит его из текста и его формата (`tests.description_format`) по той же политике, что и остальной авторский текст: `plain` экранируется и переводы строк становятся `<br>`, `richText` и `html` печатаются как есть. Гейт блока по-прежнему стоит на СТРОКЕ, поэтому пустое описание не печатает ничего. Версия МИНОРНАЯ: шаблон, связывающий только строку, работает как прежде и показывает описание без оформления. Оба поставляемых шаблона правку уже несут — узел описания стал блочным и печатается через `{{& … }}` БЕЗ `data-path`: проход по `[data-path]` заполняет узел через `textContent` после прохода DSL и затёр бы разметку. |
| 3.3.0 | 2026-09-11 | **Превью варианта умеет различать тему редактора** (§6): значением `optionPreviews` может быть не только путь, но и пара `{ light, dark }`. Лок, нарисованный под светлый фон, на тёмном редакторе нечитаем, а комплект фирменного блока обычно содержит оба варианта — теперь карточка показывает тот, что подходит теме, в которой сидит автор. Версия МИНОРНАЯ: строка по-прежнему допустима и означает одну картинку на обе темы. Значение вне этих двух форм блокирует импорт. |
...
### Умолчание решает шаблон: `default` у параметров, которые читает ядро

Часть параметров ядро читает само, а не через CSS: схему уровней (`levelScheme` и цвета своей
схемы), вид шкал и показателей, показ максимума шкалы, окраску полос подтем
(`breakdownBarFill`, §10.6). Для них `default` манифеста действует ровно так же, как для
`cssVar` и `dataAttr` ниже: если автор значение не трогал, ядро берёт умолчание ШАБЛОНА, а не
своё. Так шаблон задаёт вид теста, в котором ничего не настраивали, а редактор показывает
автору то значение, которое на самом деле применяется.

До 3.12.0 эти параметры брали жёсткое умолчание ядра, а `default` манифеста только
показывался в редакторе. У всех поставляемых шаблонов оба умолчания совпадали, поэтому
поведение прежних тестов не изменилось. Составные ключи (`progress.mode`) таким способом не
дополняются.
...
Значением может быть путь строкой ЛИБО пара под темы интерфейса:
...
Редактор берёт картинку по теме, в которой сидит АВТОР, а не по теме теста: лок,
нарисованный под светлый фон, на тёмном редакторе нечитаем. Не хватает половины пары —
берётся вторая.
...
- значение вне двух допустимых форм (не строка и не объект с `light`/`dark`) —
  блокировка: молча проигнорированное превью выглядит как пропавшая картинка;
- вариант без превью рисуется карточкой со значком-заглушкой — это нормальный случай
  для варианта со смыслом «ничего не показывать»;
...
`href`/`src` (значение заменяется на `#`) и внешний `http(s)://` в `src` — на любом теге, а
также в `href` у всех тегов, КРОМЕ `<a>` и `<area>`. Разделение по смыслу: `src` называет
ресурс, который страница грузит сама, и внешний адрес ломает автономность пакета, тогда как
`href` ссылки — направление, по которому учащийся идёт сам, и до клика не запрашивается ничего.
Внешняя ссылка поэтому СОХРАНЯЕТСЯ и получает `target="_blank" rel="noopener noreferrer"`
(атрибуты, заданные автором, не перекрываются): внутри LMS пакет живёт в кадре, и переход в том
же кадре увёл бы учащегося с незавершённой попытки. Остальная разметка проходит: форматирование,
структура, таблицы, контейнеры, атрибуты `class`/`id`/`style`, внутренние ссылки и изображения
из `/uploads/`.

Отсюда обязанность шаблона: ссылка в авторском тексте должна ЧИТАТЬСЯ как ссылка. Браузерное
умолчание не годится — оно не знает палитры шаблона, а у `<a>` без адреса его нет вовсе.
Поставляемые шаблоны объявляют правило для областей авторской разметки (обложка, текст страницы,
заметки раздела, текст вопроса, вводный блок итогов, обратная связь) в `theme.css` и одно общее
`.tb-report a` в `report.css`.
...
> хосте, внешний `http(s)`-адрес РЕСУРСА (`src`) остаётся запрещённым — в отличие от адреса
> ссылки, который сохраняется (см. выше).
...
Растровая страница сама по себе ссылок не несёт. Конвейер экспорта накладывает поверх снимка
НАСТОЯЩИЕ ссылки PDF из двух источников:

- элемент макета с классом `pdf-link-btn` и атрибутом `data-url` — по координатам элемента.
  Без этого контракта рекомендованный курс в отчёте останется картинкой;
- ссылка `<a href>` с адресом `http(s)://` или `mailto:` — как правило, из авторского текста
  (страницы документа, вводный блок, рекомендации). Область строится ПО СТРОКАМ
  (`getClientRects`), поэтому перенесённая ссылка нажимается на каждой своей строке и не
  захватывает соседний текст. Якорь `#…` и относительный путь пропускаются: в PDF им вести
  некуда.
...
> «Далее»),
> `page.sectionSubtitle` — подзаголовок раздела на экране «Введение раздела»: строка над
> авторской инструкцией (настройки `sectionSubtitleShown` и `sectionSubtitle` варианта вида
> `intro`, по умолчанию «Инструкция»). Страница вне последовательности получает пустой набор
> точек, поэтому макет с
...
> `page.nextLabel` и `page.sectionSubtitle` он задаёт настройками варианта. Все три хоста
> (SCORM-пакет, веб-хост,
> предпросмотр) читают `page.*` из одного общего ядра (`shared/template/page-sequences.ts`), так
> что считают одинаково.
>
> Две подписи различаются правилом пустоты, и это НЕ произвол. `page.nextLabel` пустым не
> бывает: кнопка без подписи — дефект, поэтому очищенное поле возвращает «Далее».
> `page.sectionSubtitle` пустым бывает — так автор убирает надпись, — но приходит это ТОЛЬКО от
> выключенного тумблера: пустое поле при включённом тумблере означает «следовать шаблону».
> Причина в редакторе: контрол настройки показывает объявленное умолчание серой подсказкой, и
> «не трогал» с «очистил» выглядят в форме одинаково, поэтому пустота не может значить
> «выключить». Макет обязан печатать надпись по условию:
>
> ```text
> {{#if page.sectionSubtitle}}<span class="tb-card__head">{{ page.sectionSubtitle }}</span>{{/if}}
> ```
>
> Блок инструкции при выключенной надписи остаётся: гасится строка, а не авторский текст.
...
| `course.description` | Описание теста строкой (стартовый экран) |
| `course.descriptionHtml` | Оно же разметкой |
...
Описание теста на стартовом экране устроено так же. Формат берётся из своей колонки
(`tests.description_format`), а не из `feedback_json`, но политика разметки та же:

```html
{{#if course.description}}<div class="tb-cover__desc">{{& course.descriptionHtml }}</div>{{/if}}
```

Элемент здесь блочный (`div`, не `p`): размеченное описание может нести абзацы и списки, а
`<p>` внутри `<p>` браузер закрывает сам. И `data-path` на этом узле стоять НЕ должен —
проход по `[data-path]` заполняет узел присваиванием `textContent` уже после прохода DSL и
затёр бы разметку, которую интерполяция только что напечатала.
...
| `passClass` | Готовый модификатор строки: `is-pass`, `is-fail` или пустая строка. Пуст и при окраске «По доле» или «Нейтральная» (см. ниже) |
| `barFill` | Готовая заливка полосы — значение CSS `background` вида `linear-gradient(...)`. Приходит только при окраске «По доле» и непустой полосе (с 3.12.0) |
...
**Окраска полосы — выбор автора теста** (с 3.12.0). Параметр оформления `breakdownBarFill`
(тип `select`, §6) принимает три значения:

| Значение | Что кодирует цвет | Что получает строка |
| --- | --- | --- |
| `verdict` | Исход подтемы | `passClass` = `is-pass` / `is-fail`; цвет задаёт CSS шаблона. Так же ведёт себя строка, когда параметр не объявлен или не задан |
| `share` | Долю | `passClass` пуст, `barFill` — градиент от неблагоприятного края схемы уровней к её цвету в точке доли |
| `neutral` | Ничего | `passClass` пуст, `barFill` нет; полоса нейтрального цвета шаблона |

Градиент строится по той же рампе, что окрашивает уровни шкал и показателей (`levelScheme` и
три цвета своей схемы): одна палитра «плохо → хорошо» на весь тест. Заливка считается от
ширины САМОЙ полосы: полоса шириной `p` показывает ровно левую часть градиента, растянутого на
всю дорожку, поэтому короткая полоса не заканчивается благоприятным цветом. Переход между
цветами — прямой, в RGB, как у обычного CSS-градиента: так полосы печатались в отчёте и до
3.12.0. Зоны уровней идут по дуге оттенка и для пары «оранжевый → бирюзовый» проходят через
жёлтый и зелёный; у полосы этого нет. Единственная промежуточная точка — середина схемы, если
она задана и полоса её переходит. Цвета выводятся как `rgb(...)` только из тех троек HSL,
которые ядро разобрало: испорченное значение своей схемы заменяется краем светофора, а не
печатается.

Макет печатает заливку в `style` той же полосы, после ширины: инлайн-значение перебивает цвет
заливки из CSS шаблона, поэтому правил под режимы шаблону не нужно. Шаблон, который `barFill`
не печатает, в режиме «По доле» покажет полосу своим нейтральным цветом — вердикт-класс снят.
Параметр действует одинаково на экране итогов (в карточке темы и в сводном блоке) и в отчёте.
...
### 10.9 Лимит времени: `course.timeLimitLabel`

Лимит времени теста приходит в контекст ПАРОЙ: число минут и готовая надпись.

| Поле | Что несёт |
| --- | --- |
| `course.timeLimitMinutes` | Лимит числом минут, как его задал автор |
| `course.timeLimitLabel` | Он же надписью: «14 дней», «1 день 2 ч», «2 ч 30 мин», «45 мин» |

Надпись строит ядро (`shared/template/duration`): раскладывает минуты в дни, часы и минуты,
опускает пустые части и склоняет «день». Единица измерения принадлежит ЯДРУ, а не макету,
потому что движок намеренно не понимает ни выражений, ни хелперов (§9): макет, печатающий
`{{ course.timeLimitMinutes }} мин`, на двухнедельном бюджете скажет «20160 мин» и другого
сказать не сможет.

Печатать надо надпись, и гейт блока ставить на неё же — она пуста ровно тогда, когда лимита
нет:

```html
{{#if course.timeLimitLabel}}
<div class="tb-facts__item">
  <div class="tb-facts__num" data-path="course.timeLimitLabel"></div>
  <div class="tb-facts__lbl">на прохождение</div>
</div>
{{/if}}
```

Число остаётся в контексте: шаблон, связывающий только его, работает как прежде. Той же
надписью печатается лимит РАЗДЕЛА (`sectionIntro.timeLimitLabel`, §7), поэтому обложка и
введение раздела одного теста не могут разойтись в написании.

Бегущий отсчёт в шапке (`#timer-display`, `#section-timer-display`) считает и печатает
рантайм, а не макет; с этой версии он растёт единицами так же — `M:SS`, `H:MM:SS` от часа и
`D д H:MM:SS` от суток.

С версии 3.13.0 рядом стоит признак `course.closesOnLeave` (PRD-67 «Закрывать раздел при
выходе»): `true`, когда выход из начатого раздела закрывает его навсегда, а в тесте без
разделов — завершает попытку. Признак истинен, только если автор включил настройку И у теста
есть хоть один лимит — общий или раздела; решает ОДНО общее правило (`testClosesOnLeave`),
которое вызывают оба хоста. Приходит на стартовый экран, чтобы макет мог предупредить ДО
начала:

```html
{{#if course.closesOnLeave}}
<p class="tb-note">Выход из раздела закроет его: вернуться и дорешать будет нельзя.</p>
{{/if}}
```

Поставляемые макеты признак не печатают: оба хоста предупреждают ученика собственными
уведомлениями ядра на первом вопросе раздела. У теста без настройки поля в контексте нет
вовсе.

### 10.10 Толкование темы и подтемы: `interpretationHtml`

Толкование — текст, который ОБЪЯСНЯЕТ результат: что проверяет тема или подтема и что стоит
за полученной оценкой. От обратной связи оно отличается не тоном, а правилом выдачи, и это
различие — предмет контракта:

| | Толкование | Обратная связь |
| --- | --- | --- |
| Когда печатается | ВСЕГДА, при любом вердикте и при его отсутствии | по своему правилу: тема не взята; доля подтемы ниже общего порога теста |
| Где стоит | ПРИ своей теме или полосе | в сводном блоке «Рекомендации», вместе с курсами и материалами |
...
| Поле | Что несёт |
| --- | --- |
| `result.topicResults[].interpretationHtml` | Готовая разметка толкования ТЕМЫ |
| `result.topicResults[].breakdown[].interpretationHtml` | То же для строки разреза (§10.6) |
| `result.topicResults[].hasBreakdownNotes` | `true`, когда хоть одна строка разреза несёт своё толкование |
| `result.topicResults[].hasAside` | `true`, когда у темы есть что показать ВТОРОЙ КОЛОНКОЙ: толкование либо строки разреза |

Разметку строит ядро — тем же правилом, что и остальной авторский текст (§10.4), — поэтому
печатать её надо через `{{& … }}`, а не через `data-path`: проход по `[data-path]` заполняет
узел через `textContent` и затёр бы абзацы.

Правила контракта:

- **Поля нет вовсе, когда текста нет.** Пустая строка заставила бы макет печатать пустой
  блок, поэтому гейт стоит на самом поле.
- **Текст ТЕСТА заменяет текст ТЕМЫ целиком**, а не дополняется им. Разрешает это ядро, и
  макет получает уже разрешённое значение: две редакции одного текста, склеенные в выдаче,
  автор нигде не видит, а участник читает противоречие.
- **В блок «Рекомендации» толкование не попадает** ни при каких настройках: иначе одна и та
  же мысль напечаталась бы на экране дважды.
- **Толкования подтем показываются по настройке ТЕСТА** («Показывать толкование подтем»).
  Выключенная настройка не стирает текстов — они просто не приезжают в контекст.
- **Два признака раскладки** (`hasBreakdownNotes`, `hasAside`) считает ядро, потому что
  движок не умеет ни перебрать строки, ни сложить два вопроса в один ответ (§9). Макет,
  который их не связывает, работает как прежде.

### 10.11 Заголовки итога: свойства варианта «Итоги теста»

Тест вправе назвать свой итог сам — тремя свойствами варианта `results.*` (`settings[]`,
§6). Это НЕ надписи словаря (§8.3.1): словарь переименовывает надписи интерфейса, общие для
всех тестов шаблона, а здесь — содержание конкретного теста.

| Ключ настройки | На что действует | Пусто означает |
| --- | --- | --- |
| `headingDocument` | `course.title` на экране итогов и в отчёте | название теста |
| `headingPassed` | `result.statusLabel` на экране, `report.verdictHeadline` в отчёте | «Пройден» / «Тест пройден» |
| `headingFailed` | там же при неуспехе | «Не пройден» / «Тест не пройден» |

Правила контракта:

- **Новых полей контекста нет.** Значения приходят в те же поля, что и раньше, — макет
  правок не требует.
- **Умолчания у экрана и у документа РАЗНЫЕ** и остаются за ядром: над шапкой документа нет
  названия теста, и одним словом она не держится.
- **Тест без вердикта заголовками исхода не переименовывается**: они утверждали бы о
  слушателе то, чего экран не утверждает (§7.3). Заголовок документа при этом действует —
  как назван итог, от вердикта не зависит.
- **Шаблон, не объявивший этих ключей, полей автору не покажет**, и тест на нём останется с
  прежними умолчаниями. Объявление — обязанность шаблона, как и у любой другой настройки
  варианта.

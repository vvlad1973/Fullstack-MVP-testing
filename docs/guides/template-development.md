...
`resultsBlockOrder`, §3.4-3.5), разрез результата в карточке темы с вердиктом ключа
...
- [Платформа SCORM-шаблонов](../specs/spec-template-platform.md) — формальная
  спецификация формата (источник истины), версия 2.0.0.
...
| `passed` | Вердикт ключа: `true` / `false` / `null`. `null` = порога нет либо ключ не выдавался; тогда строка не утверждает ничего |
| `passClass` / `statusLabel` | Готовые класс и подпись вердикта; при `passed: null` обе пусты — гейтите подпись на ней самой |

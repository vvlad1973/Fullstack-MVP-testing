...
**Версия руководства:** 3.0.0 · **соответствует спецификации формата:** 3.0.0 ·
...
- [Платформа SCORM-шаблонов](../specs/spec-template-platform.md) — формальная
  спецификация формата (источник истины), версия 3.0.0.
...
**Вердикта в строке нет.** С версии 3.0.0 подтема считается и показывается, но не судится:
поля `passed`, `passClass` и `statusLabel`, которые строка несла в 1.7.0-2.0.0, из контекста
убраны. Про исход говорит ТЕМА — карточка, внутри которой стоят строки; строка отвечает
только на вопрос «сколько». Если ваша раскладка ещё связывает снятые поля, она не сломается,
но и не напечатает ничего: класс придёт пустым, а `{{#if statusLabel}}` не откроется никогда —
уберите разметку, чтобы она не вводила в заблуждение следующего верстальщика.

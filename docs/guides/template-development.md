...
**Статус:** актуально; соответствует спецификации формата версии 1.6.0 и эталонному шаблону
...
`resultsBlockOrder`, §3.4-3.5) и разрез результата в карточке темы (`breakdown[]`, §7.3).

**Версия руководства:** 1.6.0 · **соответствует спецификации формата:** 1.6.0 ·
...
- [Платформа SCORM-шаблонов](../specs/spec-template-platform.md) — формальная
  спецификация формата (источник истины), версия 1.6.0.
...
Экран итогов состоит из четырёх подблоков — `summary` (сводка баллов), `scales`, `indicators`,
`topics`. Раздел объявляет, какие из них печатает ВАШ шаблон и в каком порядке, отдельно по
экранам:
...
`result.blocks[]` (§7.3), и макет обходит его ОДНИМ циклом вместо четырёх фиксированных секций.
...
булев флаг вида: `isSummary`, `isScales`, `isIndicators`, `isTopics`. Флаги нужны потому, что
DSL не умеет сравнивать строки — `{{#if key == "scales"}}` написать нечем.

...
Толкование темы — второй, более новый текст: он объясняет результат по теме и
печатается при ЛЮБОМ вердикте. В ящике самой темы его нет: задаётся он в тесте, во
вкладке «Обратная связь и итоги» (см. [11.6](#116-по-темам)), потому что одно и то же
знание разные тесты объясняют по-разному.
...
- Кнопка «Сохранить» одна на весь ящик: она записывает и настройки теста, и оформление,
  и полотно сценария — тремя запросами, но одним вашим действием. Отдельной кнопки
  «Сохранить оформление» больше нет.
- Рядом с ней стоит **«Показать изменения»** — список того, что вы наменяли в этом
  сеансе, поле за полем, со старым и новым значением. Полезно перед сохранением чужого
...
Оформление сохраняется общей кнопкой «Сохранить» в подвале ящика, вместе с остальными
настройками.

У НОВОГО теста, который ещё ни разу не сохраняли, вкладка работает наполовину, и об этом
говорит баннер вверху. **Шаблон выбрать можно сразу**: карточка, «Заменить шаблон» и
предпросмотр доступны, и выбранный шаблон применится к тесту в момент создания — вместе с
ним сразу встанут на место и системные страницы теста. Остальные подразделы — брендирование,
цвета, макет, вид диаграмм, облик отчёта — заперты до первого сохранения: их значения и
загружаемые файлы привязаны к уже существующему тесту. Заполните обязательные поля во
вкладке «Основное», сохраните черновик — и рельс откроется целиком.
...
- **Вопросы** — таблица заданий: «Что отвечали» (у измерительного теста — «Разброс
  ответов»), «Ответов», «Доля верных», «Пропуски», «Выдаётся», «Время, медиана»,
  «Трудность». Фильтр «Требуют ревизии» оставляет только подозрительные строки. Отсюда
  же можно уйти в реестр к прохождениям, где на задании ошиблись, и прочитать ответы
  свободным текстом.

...
3. Выберите **группу**, к которой отнести эти прохождения, или заведите новую прямо
   здесь.
4. Решите два вопроса: **«Данные уже обезличены»** (в колонке участника не ФИО, а
   идентификатор — тогда повторно он не хешируется) и **«Связать с пользователями по
   ключу»**. Если в колонке участника кириллица с пробелами, сервис заподозрит ФИО и
   предупредит: снимите флажок, иначе имена попадут в базу как есть.
...
(его же можно заполнить колонкой при массовой загрузке пользователей). Совпадение ключа
связывает прохождение с человеком; по фамилии сервис не связывает никогда и учётных
записей сам не заводит. Прохождение без совпадения остаётся в аналитике под псевдонимом.
...
- **Вопросы** — таблица заданий: «Что отвечали» (у измерительного теста — «Разброс
  ответов»), «Трудность», «Дискриминативность», «Пропуски», «Выдаётся», «Время,
  медиана», «Замысел». Фильтр «Требуют ревизии» оставляет только подозрительные строки.
  Отсюда же можно уйти в реестр к прохождениям, где на задании ошиблись, и прочитать
  ответы свободным текстом. Нажатие на дискриминативность открывает разбор этого задания
  на вкладке «Качество заданий».
- **Качество заданий** — годится ли тест как измерительный инструмент; см.
  [17.5](#175-вкладка-качество-заданий).
...
**Про «Трудность» и «Замысел».** «Трудность» показывает, какую долю балла задание в
среднем приносит: 0 — не решил никто, 1 — решили все. Раньше на этом месте стояла «Доля
верных»; у задания с частичным кредитом она была неверной, потому что считала только
...
**Исключение задания из выдачи.** В таблице заданий у каждой строки есть действие
«Исключить из выдачи»: вопрос перестаёт попадать в НОВЫЕ прохождения, оставаясь в банке
и в уже собранной статистике. Так снимают задание, которое оказалось некорректным, не
...
Кнопки шапки: «Загрузить выгрузку LMS», «Экспорт Excel» и «Прохождения в реестре» —
последняя уводит в общий реестр с уже проставленным условием по этому тесту. Отдельного
списка попыток на странице теста больше нет: два списка «кто проходил» неизбежно
разошлись бы.

### 17.5. Вкладка «Качество заданий»

Вкладка «Вопросы» отвечает на вопрос «что происходит с заданием»: как часто его
выдавали, сколько думали, что выбирали. «Качество заданий» отвечает на другой — **годится
ли задание как измерительный инструмент**, и стоит ли верить итоговому баллу.
...
- **Надёжность (альфа)** — насколько согласованно задания меряют одно и то же. Приемлемо
  от 0,70, хорошо от 0,80. Выше 0,95 — подозрение, что в тесте дубли. Под числом стоит
  подсказка, сколько заданий добавить до 0,80, а если надёжность уже выше цели — сколько
  заданий можно **снять**, не потеряв в точности. Это единственное место, где сервис
...
- **Ошибка измерения** — на сколько балл участника может отклониться от его истинного
  ...
  заданиями.
- **Под подозрением** — сколько заданий получили хотя бы один признак.
- **Заданий с надёжной оценкой** — сколько накопили сто и более наблюдений.

**Таблица заданий** отсортирована по силе подозрения: сначала прямые дефекты, потом
догадки, потом спокойные задания. Признак назван словами:
...
| На уровне угадывания | Верных не больше, чем дало бы случайное тыканье | Задание не работает: переписать или убрать |
...
| Слишком трудное / слишком лёгкое | Решают почти никто или почти все | Такое задание никого не отсеивает; оставить можно, но как «разминку» |
| Мало данных | Наблюдений меньше тридцати | Ничего не делать: подпись говорит, сколько ещё нужно |

Нажатие на строку открывает **разбор задания**: трудность, трудность с поправкой на
...
Если задание правили, разбор покажет **редакции**: наблюдения до правки и после не
складываются, и по каждой редакции числа свои. Так проверяют, стало ли задание лучше.
...
тридцати, а «надёжно» они становятся со ста. У задания с дюжиной ответов трудность
будет, а дискриминативности не будет.

**Когда числа предупреждают о себе сами.** Над таблицей могут появиться баннеры:
выдача неоднородна (при случайной выдаче или адаптиве задания видели разные люди),
заметная доля наблюдений пришла из импорта (там известен только факт «верно / неверно»),
часть участников могла посчитаться дважды (смешаны сырые и предобезличенные выгрузки).
Числа при этом остаются полезными для поиска плохих заданий, но сравнивать их с другим
тестом уже нельзя.

**Две выгрузки** внизу: «Психометрический отчёт» — книга Excel со всеми числами по
заданиям и по тесту, «Матрица ответов» — таблица «участники × задания» для расчётов во
внешнем пакете.

У теста, где все задания измерительные (опросник), вкладка честно говорит, что проверять

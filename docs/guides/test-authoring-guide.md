...
про ОДИН тест: как он устроен и где в нём проблемные вопросы.
...
### 17.3. Раздел «Аналитика»: три вкладки
...
догружаются порциями, сортировка — по любому столбцу. Набранные условия можно
сохранить двумя способами: «Сохранить фильтр» — чтобы вернуться к ним из списка
«Сохранённые» (тестов в фильтре сколько угодно), «Сохранить как срез» — чтобы сравнивать
(срез всегда внутри одного теста).

Кнопка **«Экспорт»** выгружает книгой Excel ровно то, что отобрано, — второго набора
условий в окне выгрузки нет. В окне выбираются только листы книги («Прохождения», «Ответы
по вопросам», «Сводка», «Статистика вопросов», у адаптивных — «Статистика уровней», а также
«Рекомендации») и правило «Только лучшая попытка участника». Отдельной вкладки
«Экспорт» больше нет.

**Срезы** — сравнение групп прохождений внутри одного теста. Сначала выберите тест и,
если нужно, период: это рамка расчёта, общая для всех срезов. Режима два:

- **«Список срезов»** разбивает прохождения по одной оси. Осей семь: «Группа», «Поток
  (период)», «Номер попытки», «Версия теста», «Вариант выдачи», «Источник», «Внутренние и
  внешние». Меню «⋯» в строке: «Открыть прохождения» (реестр с условиями этой строки),
  «Аналитика теста» (страница теста с теми же условиями — чтобы увидеть, что именно у
  этой группы не получилось), «Сравнить с другим срезом», «Сохранить как срез»,
  «Выгрузить прохождения». У строки, которую условиями реестра не описать (например,
  «Без группы» или номер попытки), сравнения и сохранения нет.
- **«Сравнение»** ставит рядом до четырёх срезов: сохранённые, «Тест целиком» и
  набранный отбор. У сохранённого среза можно «Изменить условия» прямо в слоте. Срез,
  отправленный сюда из строки списка, подписан своим именем.

Сравнить набранный отбор со срезом можно и НЕ сохраняя его — кнопка «Сравнить со
срезом» в реестре.
...
- **Обзор** — четыре плитки: «Прохождений», «Сдали», «Средний результат», «Время,
  медиана». У теста, который ничего не оценивает, плитки честно говорят «тест не
  оценивает ответы» и «вердикт не выносится» вместо нулей. Под ними — распределение
  результатов корзинами одной ширины с вертикалью проходного балла, разбор по темам и
  динамика прохождения во времени.
...
Кнопки шапки: «Загрузить выгрузку LMS», «Экспорт Excel», «Прохождения теста» и значок
«Обновить» (перечитывает числа с сервера). «Прохождения теста» открывает вкладку «Прохождения» раздела «Аналитика» с
...
| Вопрос ещё не выдавался | Вопрос входит в выдачу теста, но не попал ни в одно прохождение | Ничего, если выдача случайная и прохождений мало; если прохождений много — проверить квоты и варианты |

В таблице — все вопросы, которые тест МОЖЕТ выдать, а не только те, на которые уже
ответили: банк тем разделов без исключённых из выдачи, у раздела с вариантами — вопросы
вариантов, у адаптивного теста — вопросы уровней. Вопросы, которые ещё ни разу не
выдавались, стоят в конце и попадают в «Мало данных»; на надёжность и отметки они не
влияют.
...
внешнем пакете. В отчёте — те же вопросы, что на экране, включая ещё не выданные (у них
0 наблюдений и отметка «вопрос ещё не выдавался»). В матрицу они не входят: колонка без
единого ответа внешнему пакету ничего не даёт.

**Сравнение срезов.** Ссылка «Сравнить срезы» над вкладкой ставит рядом до четырёх
выборок — те же срезы, что в разделе «Аналитика», — и показывает две таблицы:
«Надёжность и ошибка измерения» и «Трудность вопросов». Так видно, одинаково ли тест
работает, например, в разных группах или до и после правки. Вернуться к одной выборке —
переключателем «Одна выборка / Сравнение» в шапке карточки.
...
**Шкалы методики.** Сначала сводка: сколько пунктов в каждой шкале, альфа Кронбаха и вывод
по шкале. Ниже — карточка на каждую шкалу со своими пунктами. В подзаголовке карточки
сказано, по сколько градаций у ответов («Пять градаций ответа», «от 5 до 7 градаций
ответа») и сколько прохождений учтено. У каждого пункта — вклад в шкалу со знаком,
корреляция с остатком шкалы и распределение ответов. Распределение всегда одного вида:
столбики пронумерованы, словами подписаны только крайние градации, а наведение на
столбики показывает расшифровку номеров списком с долями. Колонка «Качество пункта»
отмечает пункт, который работает против шкалы (чаще всего у обратного пункта забыли
поставить отрицательный вклад), и «мёртвый» пункт, где почти все выбирают одну градацию.

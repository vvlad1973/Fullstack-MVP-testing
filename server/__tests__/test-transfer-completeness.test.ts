...
const NOT_CARRIED: Record<string, string> = {
  ...
  test_review_comments:
    "комментарии рецензентов — разговор ЛЮДЕЙ этой инсталляции: автор, рецензент, время и исход ветки. На приёмнике этих учётных записей нет, а якорь комментария ссылается на сущность с пином содержимого, которое там пересоздаётся заново. Переносить их значило бы привезти чужое обсуждение без его участников (PRD-52).",
  lms_import_batches:
    "партии загруженных выгрузок отчётов LMS — данные ИНСТАЛЛЯЦИИ, как и попытки с телеметрией " +
    "рядом (PRD-54). Партия хранит, кто и когда залил файл этой установки, и держит прохождения, " +
    "которых на приёмнике нет: перенос привёз бы записи об откате того, что там не лежит.",
  question_exposure:
    "счётчик выдач заданий — данные ИНСТАЛЛЯЦИИ, как попытки и телеметрия, из которых он и " +
    "собирается (PRD-55). Экспозиция описывает, сколько раз задание показали ЗДЕСЬ: привезти её " +
    "на приёмник значило бы утверждать, что тамошние участники видели задание столько-то раз. " +
    "Счётчик восстановим пересчётом из фактов приёмника (`npm run exposure:rebuild`), а до " +
    "накопления данных выдача там просто работает равномерно — штатная деградация поправки.",
  analytics_slices:
    "сохранённые срезы прохождений — рабочие заметки АНАЛИТИКА этой инсталляции (PRD-56 FR-07b): " +
    "срез хранит условия отбора — группы, источники, исходы — и все они указывают на сущности, " +
    "которых на приёмнике нет. Привезти срез значило бы привезти отбор, под который там не " +
    "подойдёт ни одно прохождение, и назвать пустую выборку сохранённой аналитикой.",

...
import { ANSWER_KIND_CHOICES, JOIN_CHOICES, PROMPT_FORMAT_CHOICES } from "./workbook-answer-rules";
import {
  QUESTION_HEADERS,
  QUESTION_WIDTHS,
...
const VALIDATED_COLUMNS: Record<string, Record<string, string[]>> = {
  [SHEET_QUESTIONS]: {
    ...
    // PRD-57: свойства текстового ответа. Два перечисления из двух слов каждое —
    // именно тот случай, где список избавляет от опечатки, ценой которой была бы
    // молчаливая подмена вида эталона.
    "Формат текста": PROMPT_FORMAT_CHOICES,
    "Вид ответа": ANSWER_KIND_CHOICES,
    "Связка правил": JOIN_CHOICES,
    "Ответ обязателен": BOOL_CHOICES,
  },
  ...
  [SHEET_FEEDBACK]: {
    ...
    // Толкование пишется тем же редактором, что и обратная связь, поэтому список форматов
    // у него ТОТ ЖЕ — своя копия подписей разошлась бы с первой при первой же правке.
    "Формат толкования": FEEDBACK_FORMAT_CHOICES,
  },
...
export const EXAMPLE_ROWS: Record<string, Record<string, unknown>[]> = {
  ...
  [SHEET_FEEDBACK]: [
    {
      // У ТЕСТА толкования нет: толкуется раздел или подтема, а не тест целиком, — поэтому
      // обе колонки толкования в этой строке пусты, и заполненная отвергается при загрузке.
      "Кому": "Тест",
      "Раздел": "",
      "Подтема": "",
      "Формат": "Простой",
    ...
    {
      // Два текста одного владельца в одной строке. Толкование объясняет результат и
      // печатается ВСЕГДА; обратная связь советует и выдаётся по своему правилу.
      "Кому": "Раздел",
      "Раздел": "Финансы",
      "Подтема": "",
      "Формат": "Форматированный",
      "Текст": "<p>Разберите отчёт о прибылях и убытках на примере своей компании.</p>",
      "Формат толкования": "Простой",
      "Толкование": "Читает управленческую отчётность и объясняет её выводы команде.",
    },
    {
      ...
      "Формат толкования": "Простой",
      "Толкование": "Состав форм отчётности и сроки их сдачи.",
    },
  ],
...
const HELP_ROWS: HelpRow[] = [
  ...
  [SHEET_QUESTIONS, "Тип вопроса", "да", "Одно из: multiple_choice (одиночный выбор) | multiple_response (множественный) | matching (сопоставление) | ranking (ранжирование) | scale (шкала) | allocation (распределение баллов) | short_answer (короткий ответ) | fill_in_blanks (пропуски) | long_answer (развёрнутый ответ). От типа зависит формат двух следующих колонок.", "multiple_choice"],
  ...
  [SHEET_QUESTIONS, "Формат текста", "нет", "В каком режиме набран текст задания: разметка (по умолчанию, подмножество markdown) | форматированный | html. Пусто — разметка, то есть так, как написаны все задания, заведённые раньше. Листинг, формула и пропуск работают во всех трёх режимах; в html листинг пишется как <pre><code class=\"language-sql\">.", "html"],
  ...
  [SHEET_QUESTIONS, "Вид ответа", "нет", "Только для типов, где ученик ПИШЕТ: текст (по умолчанию) или число. У числа сравниваются значения, а не написание: «3,14», «3.14» и « 3,14 » — один ответ. Пусто — вид определяется по самим правилам: все правила с оператором означают число.", "число"],
  [SHEET_QUESTIONS, "Связка правил", "нет", "Только для типов, где ученик пишет: любое (по умолчанию — достаточно одного правила) или все (нужны все сразу). Диапазон записывается двумя правилами и связкой «все»: «>= 10» и «<= 20».", "все"],
  [SHEET_QUESTIONS, "Единица измерения", "нет", "Только для короткого ответа с числом: подпись рядом с полем, до 16 символов. Ученик её не набирает — она показывается.", "°C"],
  [SHEET_QUESTIONS, "Предел длины", "нет", "Короткий и развёрнутый ответ: сколько символов можно набрать. Целое больше нуля. Пусто — системный предел.", "4000"],
  [SHEET_QUESTIONS, "Подсказка в поле", "нет", "Только для развёрнутого ответа: текст, который виден в пустом поле и исчезает при вводе.", "Ответьте своими словами"],
  [SHEET_QUESTIONS, "Ответ обязателен", "нет", "Только для развёрнутого ответа: «да» — без ответа дальше не пустит. Пусто или «нет» — задание можно пропустить.", "да"],
  ["", "", "", "ПРАВИЛА ПРОВЕРКИ у текстовых типов пишутся в колонке «Номера правильных ответов», по одному в строке ячейки (перенос строки внутри клетки — Alt+Enter). Текстом — как сравнивается, со звёздочкой вместо любого отрезка: «Федеральная служба по * надзору». Регулярным выражением — с префиксом «регулярное:». Числом — оператором и числом: «= 3,14 ± 0,01», «>= 10», «<> 0».", "Ростехнадзор"],
  ["", "", "", "ПРОПУСКИ ставятся в тексте задания двойными фигурными скобками: «Столица России — {{city}}». Правило относится к пропуску именем в квадратных скобках: «[city] Москва». Строка «[city: число, все]» задаёт вид ответа и связку самого пропуска, строка «[city]» сохраняет пропуск, у которого правил ещё нет. Имя, которого нет в тексте, — ошибка строки.", "[city] Москва"],
  ["", "", "", "У РАЗВЁРНУТОГО ОТВЕТА колонка правил остаётся ПУСТОЙ: автоматической проверки у типа нет, задание не приносит баллов, а попытка с ним помечается предварительной — до проверки человеком.", "(пусто)"],
  ...
  ["", "", "", "«Показывать толкование подтем» — «Да» | «Нет» (по умолчанию нет). Тексты задаются на листе «Обратная связь», колонкой «Толкование»; выключенный показ их не стирает — участнику они просто не печатаются.", "Нет"],
  GAP,
  ...
  [SHEET_FEEDBACK, "Формат толкования", "нет", "Как читать толкование: те же значения, что у «Формата» слева. Заполняется только вместе с «Толкованием».", "Простой"],
  [SHEET_FEEDBACK, "Толкование", "нет", "Текст, который ОБЪЯСНЯЕТ результат: что проверяет раздел или подтема и что стоит за полученной оценкой. Печатается ВСЕГДА — и когда взято, и когда нет, — и в блок «Рекомендации» не попадает. Этим оно и отличается от «Текста» слева, который советует и выдаётся по своему правилу. Для «Кому» = «Тест» ячейка должна быть ПУСТОЙ: толкуется раздел или подтема, а не тест.", "Читает управленческую отчётность и объясняет её выводы команде."],
  ["", "", "", "Толкование из книги пишется НА УРОВЕНЬ ТЕСТА: оно переопределяет толкование самой темы, а тему книга не трогает — одна и та же тема стоит во многих тестах, и правка из книги одного теста меняла бы чужие.", ""],
  ["", "", "", "Пустое «Толкование» у названного владельца снимает переопределение: участник снова читает толкование темы. Так же, как пустой «Текст» снимает обратную связь.", ""],
  ["", "", "", "Показ толкований ПОДТЕМ включается отдельно — настройкой теста «Показывать толкование подтем» (лист «Настройки»). Пока он выключен, тексты хранятся, но участнику не печатаются.", ""],
  ...
  [SHEET_PAGES, "Скрыт", "нет", "да — экран остаётся в тесте, но ученику не выдаётся. нет (по умолчанию) — выдаётся. Пусто — видимость не меняется. Скрыть нельзя «Вопросы» и «Маршрутизатор»: без них прохождение не состоится.", "нет"],
  ...
  [SHEET_OUTCOMES, "Рекомендации", "нет", "Текст рекомендаций исхода. Пустая ячейка ОЧИЩАЕТ сохранённый текст. Ссылки, мероприятия и вложения рекомендаций книга не переносит — при загрузке они сохраняются как были.", "Проведите индивидуальные беседы"],
  ["", "", "", "У показателя, который определяет профиль по группе шкал, «Код» — это НАБОР ключей шкал через «+» в порядке шкал теста: cel+pro. Порядок внутри набора при поиске не важен. Отдельный код count:2 — запасной текст для любого набора из двух шкал; он срабатывает, когда точного набора в списке нет.", "cel+pro"],
  GAP,

 ...
 *
 * Перевод честный до отказа: там, где у реестра такого условия нет (номер попытки, версия
 * публикации, вариант выдачи, внешний участник), возвращается пусто, и переход открывает реестр
 * по одной рамке — тесту и периоду. Подменять невыразимое условие похожим значило бы показать
 * ДРУГУЮ выборку под именем среза: числа разошлись бы, и объяснить это было бы нечем.
 */
export function registryConditions(
  axis: SliceAxis,
  key: string,
): { groupIds?: string[]; sources?: string[]; from?: string; to?: string } {

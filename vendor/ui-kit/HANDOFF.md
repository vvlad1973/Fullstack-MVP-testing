...
**Статус:** не исправлено. Замечено 2026-09-22, повторно подтверждено приёмкой аналитики
2026-09-26.
...
**Как чинить** (проверено в браузере 2026-09-22, правка тогда откачена как выходящая за объём
задачи):

1. Завести ref-флаг «выбор только что сделан»; ставить его в обработчике выбора в одиночном
   режиме перед возвратом фокуса.
2. В `onFocus` поля ввода: если флаг стоит — сбросить его и список НЕ открывать.
3. В обработчике щелчка по `.ou-combo__control` флаг сбрасывать, чтобы осознанный щелчок
   открывал список, как прежде.

Множественный режим не трогать: там список после выбора должен оставаться открытым.

**Как проверить.** В jsdom дефект виден по тому, что текст выбранного варианта находится
ДВАЖДЫ — в поле и в строке снова открытого списка (`getByText` падает на «multiple elements»).
Тест на исправление: после выбора варианта в одиночном режиме `role="listbox"` отсутствует, а
щелчок по полю снова открывает список.
